...
7 阶段流程（preclean → segment → rule_gate → judge → verify → answer_gate → post）、
输出去向、真实运行数据漏斗、双分类体系（复杂 9 类 + 普通 16 类）与全部生产提示词。
...
└── src/                          # 源部件（修改只动这里，改完跑 build.py）
    ...
    │   ├── 03-pipeline.css       #   流程图 / 7 阶段决策表 / 产物栏 / 数据漏斗
    ...
    │   ├── 10-pipeline.html      #   流程 / 7 阶段步骤表 / 产物 / 数据漏斗
...
提示词面板**不手写**：`build.py` 构建时用 `ast` 从
`src/query_pipeline/prompts/*.py` 提取常量自动生成（segment / value_gate /
complexity_gate / verify_complex / translate），verify 提示词按生产逻辑
（`prompts/assemble.py::build_verify_prompt`）追加已确认负例——
页面展示的提示词与代码永远同步。
