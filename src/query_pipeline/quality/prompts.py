...
def build_judge_system_prompt() -> str:
    categories = "\n".join(
        f"- {cid} {CATEGORIES[cid]}（{ENGLISH_CATEGORIES[cid]}）" for cid in sorted(CATEGORIES)
    )
...
def build_judge_payload(row: dict[str, Any]) -> dict[str, Any]:
    ...
    category_id, _, _ = category.partition("-")
    if category_id in CATEGORIES:
        label_definition = f"{category_id} {CATEGORIES[category_id]}（{ENGLISH_CATEGORIES[category_id]}）"
    else:
