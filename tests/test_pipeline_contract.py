...
from query_pipeline.models.records import CATEGORIES, parse_unified_label_response
...
class PipelineContractTest(unittest.TestCase):
    ...
    def test_unified_llm_label_stage_uses_nested_output(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            input_path = tmp_path / "input.jsonl"
            _write_jsonl(
    ...
    def test_unified_label_parser_accepts_all_category_ids(self) -> None:
        for category_id, category_name in CATEGORIES.items():
            ...
            parsed = parse_unified_label_response(raw)
